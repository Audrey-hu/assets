...
首页每天显示一句（共 60 句，一轮 60 天不重复）：古典名句带出处，其余是写给长期主义者的短句。

它是**按天推进的伪随机**，不是真随机 —— 真随机意味着刷新两次看到两句、两台设备不一样。
现在同一天里谁打开、刷新多少次、在哪台设备上，看到的都是同一句；当天这句不喜欢，
可以点「换一句」，手动选过的那句当天会记住，第二天自动回到当天的句子。

算法在 `src/lib/quotes.ts`：以固定步长（与句库总量互质）按天索引，
保证一轮走完全部句子且相邻两天不撞车；规则有 `pnpm test:quotes`（17 条）守着。
...
- **默认不生成示例数据**：第一次打开是空白的。示例数据只是演示用的假记录，
  需要主动到「我的 → 重新开始 → 载入示例数据」加载；任何设备打开链接看到的都是空白，
  不会看到别人的（或看起来像真人的）记录
- 照片存进私有存储桶，按 id 对账：本地缺就下载，云端缺就上传
- 数据权限由 Supabase 的行级权限（RLS）保证，每个账号只能读写自己的行

三组测试：`pnpm test:sync`（合并规则 27 条）、
`pnpm test:db`（墓碑并发写 4 条，防止「删掉的记录又回来」）、
`pnpm test:quotes`（今日一句的取句规则 17 条）。
